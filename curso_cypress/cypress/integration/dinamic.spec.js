...
describe ('Tests dinamic', () => {
    ...
    foods.forEach(food => {
        it(`Cadatro com cumida ${food}`, () => {    //comcatenamos com o IT para pegar o titulo de cada um no test
            ...
            cy.xpath(`//label[contains(., '${food}')]/preceding-sibling::input`).click() // concatenamos aqui tbm para clicar em cada um.
    ...
    it.only('Deve usar todos usando o Each', () => {
        cy.get('#formNome').type('Qualquer')
        cy.get('#formSobrenome').type('Show')
        cy.get(`[name=formSexo][value=M]`).click() 
        cy.get('[name="formComidaFavorita"]').each($el => { //usamos esse comando para setarmos o valor
           //$el.click()
           if ($el.val() != 'vegetariano') //pedimos para que se ouvesse algum elemento com esse valor nao deve clicar
            cy.wrap($el).click()
