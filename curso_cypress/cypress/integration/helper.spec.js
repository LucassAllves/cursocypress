...
describe ('Helper', () => {
    it('Wrap', () => {// wrap serve para encpsular
        const obj =  {nome: "User", idade: 20}//definimos os valores das variaives 
        expect(obj).to.have.property('nome')// inspecionamos de exite o velor nome 
        cy.wrap(obj).should('have.property', 'nome')//aqui validamos usando o should com wrap pois sem ele da erro.
  
        cy.visit('https://www.wcaquino.me/cypress/componentes.html ')
        cy.get('#formNome').then($el => {
            //$el.ver('funciona???')esse é um comando Jquery so que precisamos rodar no cypress
        ...
        cy.get('#buttonSimple').then(() => console.log('Encontrei o Primeiro Botao'))// pegamos dois valores e imprimimos no console 
    ...
    it('Its', () => {
        ...
        cy.wrap(obj2).its('endereco').should('have.property', 'rua')// tudo validação com its
    ...
    // rstamos PRENDENDO A INVOKAR UMA FUNÇÃO
    it.only('Invok', () => {
        ...
        cy.window().invoke('alert', 'Da pra Ver?')// peguei o window da pagina e invokey uma msg de alert
        cy.get('#resultado')
        .invoke('html', '<input type="button", value="hached!"/>')// peguei o valor e invokei de fosse mudado o HTML da pagina usando esses valores.
    })
}) 