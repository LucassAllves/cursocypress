...
beforeEach(() => {
  ...
  it("Text", () => {
    ...
    ) // e aqui estamos indo direto no elemento do Cuidado
  })

  it("links", () => {
    cy.get('[href="#"]').click()
    cy.get("#resultado").should("have.text", "Voltou!")
    // vamos dar um reload na tela e ver se texto msg volta
    cy.reload()
    cy.get("#resultado").should("have.not.text", "Voltou!") // verificamos se nao tem msg na tela
  ...
  it("TextFields", () => {
    ...
    cy.get("#formNome").should("have.value", "Cypress Test") // sempre que buscar um texto pelo Value(valor)
    cy.get("#elementosForm\\:sugestoes") // sempre que tiver problemas com : é so colocar //
      .type("Textearea")
  ...
  it("comboBox", () => {
    cy.get("[data-test=dataEscolaridade]") // mapeamos o elemento
      ...
      .should("have.value", "2graucomp") // verificamos se ele foi selecionado.
    // validar todas as opçoes do combo.
    ...
    cy.get("[data-test=dataEscolaridade] option").then(($arr) => {
      ...
      expect(values).to.include.members(["Superior", "Mestrado"]) // inspecionei os valores e validei se tinha mestrado e superior
    })
  })
  // aqui selecionei mais de uma opção na pagina do combobox, mais uma forma de validar
  it("ComboMultiplo", () => {
    cy.get("[data-testid=dataEsportes]").select(["natacao", "Corrida", "nada"])
    cy.get("[data-testid=dataEsportes]").then(($el) => {
      //guardamos o valor dentro de $el
