...
describe("Work whit alerts", () => {
  ...
  it("Alerts com mock", () => {
    ...
    cy.on("window:alert", stub); // Ou seja toda vez que o evento acontecer ira trazer a mesagem da variavel msg
    cy.get("#alert")
      .click()
      .then(() => {
  ...
  it("Confirm (Confirmação)", () => {
    cy.on("window:confirm", (msg) => {
      //Nunca se esquecer de mudar o (window:confirm/alert)
  ...
  it("Prompt ", () => {
    cy.window().then((win) => {
      //usamos o window para pegar toda a pagina e colocamos promises then
      cy.stub(win, "prompt").returns("42"); // cypress buca o window da pagina e pega o prompt e returns 42
    });

    cy.on("window:confirm", (msg) => {
      //Nunca de esquecer de mudar o (window:confirm/alert)
